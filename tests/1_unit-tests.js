...
suite('Unit Tests', function() {
    ...
    suite('Function convertHandler.convert(num, unit)', function() {
        test('Gal to L', function(done) {
            ...
            assert.approximately(convertHandler.convert(input[0],input[1]),expected,0.1); // 0.1 is the tolerance
            done();
        });

        test('L to Gal', function(done) {
            var input = [7.65,'L'];
            var expected = .0209171529636155660813491801416;
        ...
        test('Km to Mi', function(done) {
            ...
            assert.approximately(convertHanlder.convert(input[0],input[1]), expected, 0.1)
            done();
        });
